...
AWS deployment for the Nag backend: API Gateway HTTP API → Lambda (`dotnet10`, arm64) → Aurora PostgreSQL Serverless v2 (engine 17). API key and DB password in AWS Secrets Manager.
...
Internet → API Gateway HTTP API ($default)
        ...
        → Secrets Manager (API key + RDS-managed master secret)
        → CloudWatch Logs /aws/lambda/nag-api (14-day retention)
```

The Lambda fetches the RDS master password and the API key from Secrets Manager at cold start — see `backend/Nag.Api/Program.cs`. No plaintext secrets in env vars.
...
# optional: pulumi config set nag:dbMinAcu 0.5
...
infra/
  ...
  src/
    ...
    network.ts            # VPC, subnets, NAT, SGs
    database.ts           # Aurora Serverless v2 cluster + writer
    secrets.ts            # API-key secret
